...
- Basic terminology:
    ...
    - Client -> The application that wants to perform actions on behalf of the Resource Owner.
    - Authorization Server -> The application that knows the Resource Owner, where he has an account.
    - Resource Server -> The API or service the Client wants to use on behalf of the Resource Owner.
    ...
    - Response Type -> The type of information the Client expects to receive. Generally a Authorization Code.
    - Scope -> Granular permissions that the Client wants.
    - Consent -> The Authorization Server takes the Scopes the Client is requesting, and verifies if the Resource Owner want to give the Client permission.
    - Client ID -> ID used to identify the Client with the Authorization Server.
    - Client Secret -> Secret password that only the Client and Authorization Server knows.
    - Authorization Code -> A temporary code the Client receives from the Authorization Server in order to receive an Access Token.
    - Access Token -> The key the client will use to communicate with the Resource Server. This is what gives the Client permission to perform actions with the Resource server on behalf of the Resource Owner.

- Basic example:
    1. The Resource Owner (an User) wants to allow the Client (the app the User is using) to access their data on the Resource Server (maybe another App).
    2. The Client directs the User to the Authorization Server (e.g, login page) with the Client ID, Redirect URI and Scope.
    3. The User logs in (or ir already logged in) and consents to the requested Scopes (allows the Client to have access to something or behave on behalf of the User).
    4. The Authorization Server redirects the User back to the Client's Redirect URI with an Authorization Code.
...
> Draw.io (future diagrams.net) and Google Drive.
    ...
    4. draw.io exchanges the Authorization Code for an Access Token.
